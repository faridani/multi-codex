...
def ensure_app_dirs(repo_slug: str) -> tuple[Path, Path]:
    """Create/return the app directories for clones and reports."""
    home = Path.home()
...
def build_architecture_report(branch_markdown: str) -> str:
    """Return the architecture deep-dive prompt for a branch."""
    return build_single_branch_prompt(prompts.load_prompt("arch_deep_dive"), branch_markdown)


def build_feature_security_report(branch_markdown: str) -> str:
    """Return the feature/security/modernization prompt for a branch."""
    return build_single_branch_prompt(
        prompts.load_prompt("feature_security_analysis"), branch_markdown
    )
...
def build_pr_mega_prompt(
    ...
    if diff_result.ok and diff_result.has_changes:
        diff_block = ["```diff", diff_result.diff_text, "```"]
    elif diff_result.ok:
        message = diff_result.message or (
            f"No differences between origin/{branch_name} and origin/{base_branch}."
        )
        diff_block = ["```text", message, "```"]
    else:
        message = diff_result.message or "Unable to compute diff."
        diff_block = ["```text", f"Error computing diff: {message}", "```"]
