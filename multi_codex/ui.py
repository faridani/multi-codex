...
def save_report(text: str, report_dir: Path, name: str) -> Path:
    """Save a markdown report inside the report directory."""
...
def main() -> None:
    ...
    try:
        existed = core.ensure_local_clone(repo_url, repo_path)
    ...
    if selected_option == options[0]:
        ...
        for branch_name, bs in branch_specs.items():
            ...
            branch_slug = core.slugify_branch_name(branch_name)
            branch_md_path = save_report(md_text, report_path, f"branch_{branch_slug}.md")
        ...
        combined_prompt_path = save_report(
            combined_prompt, report_path, "combined_spec_and_branches.md"
        )
        ...
        output_path = save_report(
            combined_prompt, report_path, f"pr_review_prompt_{branch_slug}_vs_{base_slug}.md"
        )
        ...
        combined_prompt = core.build_feature_security_analysis(repo_path, branch_name)

        branch_slug = core.slugify_branch_name(branch_name)
        output_path = save_report(
            combined_prompt, report_path, f"feature_security_report_{branch_slug}.md"
        )
