...
def ask_yes_no(prompt: str, default: bool = False, suffix: Optional[str] = None) -> bool:
    """Ask a yes/no question, returning True for yes and False for no."""
...
def prompt_for_branch_selection(repo_path: Path, action_label: str) -> str:
    ...
    try:
        ...
        print(color_text(f"Failed to fetch branches: {exc}", "yellow"))
...
def save_report(report_dir: Path, name: str, content: str) -> Path:
    """Save a markdown report under the user's report directory."""
...
def monitor_branches(repo_path: Path) -> Dict[str, core.BranchSpec]:
    """Poll remote branches on origin and ask the user which to evaluate."""
    selected: Dict[str, core.BranchSpec] = {}
    seen_branches = set()
        ...
        while True:
            ...
            for branch in new_branches:
                ...
                if ask_yes_no(add_prompt, default=True, suffix=f" {color_text('[Y/n]:', 'grey')} "):
                    selected[branch] = core.BranchSpec(name=branch)
                    print(
                        color_text(
...
def main() -> None:
    ...
    if selected_option == options[0]:
        ...
        branch_markdown = core.collect_branch_markdown(repo_path, branch_name)
        combined_prompt = core.build_architecture_report(branch_markdown)

        branch_slug = core.slugify_branch_name(branch_name)
        output_path = save_report(report_path, f"architecture_report_{branch_slug}.md", combined_prompt)

        print(f"Architectural report prompt saved to: {output_path}")
        ...
        for branch_name, bs in branch_specs.items():
            ...
            branch_slug = core.slugify_branch_name(branch_name)
            branch_md_path = save_report(report_path, f"branch_{branch_slug}.md", md_text)
            bs.branch_markdown_path = str(branch_md_path)
            print(f"  -> Branch markdown saved to: {branch_md_path}")

        document_body = core.build_document_body(spec_path, spec_content, branch_markdown)
        branch_names_sorted = sorted(branch_specs.keys())

        combined_prompt = core.build_final_prompt(document_body, branch_names_sorted)
        combined_prompt_path = save_report(report_path, "combined_spec_and_branches.md", combined_prompt)

        print(color_text("\nCombined markdown saved to:", "magenta", bold=True))
        print(f"  -> Path: {combined_prompt_path}")
        ...
        if copied:
            print(color_text("  • Combined prompt copied to your clipboard.", "green"))
        else:
            print(
                color_text(
        ...
        print(f"  - Combined specs + branches prompt: {combined_prompt_path}")
        ...
        output_path = save_report(
            report_path, f"pr_review_prompt_{branch_slug}_vs_{base_slug}.md", combined_prompt
        )
        print(f"PR review mega prompt saved to: {output_path}")
        if copy_to_clipboard(combined_prompt):
            print(color_text("  • PR mega prompt copied to your clipboard.", "green"))
        else:
            print(
                color_text(
        ...
        branch_markdown = core.collect_branch_markdown(repo_path, branch_name)
        combined_prompt = core.build_feature_security_report(branch_markdown)

        branch_slug = core.slugify_branch_name(branch_name)
        output_path = save_report(
            report_path, f"feature_security_report_{branch_slug}.md", combined_prompt
        )
        print(f"Feature and security report saved to: {output_path}")
        if copy_to_clipboard(combined_prompt):
            print(
                color_text(
                    "  • Prompt copied. Upload the saved file or paste into your AI and ask it to follow the file instructions.",
