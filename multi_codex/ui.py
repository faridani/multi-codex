"""CLI and user interaction layer for multi-codex."""
...
def prompt_for_branch_selection(repo_path: Path, action_label: str) -> str:
    ...
    try:
        ...
        print(color_text(str(exc), "yellow"))
    ...
    while True:
        ...
        if raw:
            ...
            try:
                ...
                with open(expanded, "r", encoding="latin-1") as file:
                    content = file.read()
                return expanded, content
            except Exception as exc:  # noqa: BLE001
                print(f"Error reading spec file: {exc}\n")
                continue
...
def save_report(text: str, repo_path: Path, report_dir: Path, name: str) -> Path:
    """Save a generated report in the user report directory."""
    report_path = report_dir / name
    report_path.write_text(text, encoding="utf-8")
    print_saved_file("  -> Saved", report_path)
    return report_path
...
def main() -> None:
    ...
    try:
        core.ensure_local_clone(repo_url, repo_path)
        print(color_text(f"Using local clone at: {repo_path}", "grey"))
    except RuntimeError as exc:
        print(color_text(str(exc), "yellow"))
        sys.exit(1)
    ...
    if selected_option == options[0]:
        ...
        output_path = save_report(
            combined_prompt, repo_path, report_path, f"architecture_report_{branch_slug}.md"
        )
        ...
        for branch_name, bs in branch_specs.items():
            ...
            branch_slug = core.slugify_branch_name(branch_name)
            branch_md_path = save_report(
                md_text, repo_path, report_path, f"branch_{branch_slug}.md"
            )
        ...
        combined_prompt_path = save_report(
            combined_prompt, repo_path, report_path, "combined_spec_and_branches.md"
        )
        ...
        output_path = save_report(
            ...
            repo_path,
            report_path,
            f"pr_review_prompt_{branch_slug}_vs_{base_slug}.md",
        )
        ...
        branch_slug = core.slugify_branch_name(branch_name)
        output_path = save_report(
            combined_prompt, repo_path, report_path, f"feature_security_report_{branch_slug}.md"
        )
